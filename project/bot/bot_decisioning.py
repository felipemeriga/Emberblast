...
def move(self) -> None:
    ...
    if self.current_play_style == IPlayingMode.DEFENSIVE:
        safer_positions_map = {self.current_bot.position: 0}

        for possibility in possibilities:
            average_distance = self.game.game_map. \
                graph.get_average_distance_source_destinations(possibility,
                                                               remaining_players_positions)
            safer_positions_map[possibility] = average_distance
    ...
    if self.current_play_style == IPlayingMode.AGGRESSIVE:
        ...
        if possible_foe is None:
            aggressive_positions_map = {}
            for possibility in possibilities:
                average_distance = self.game.game_map. \
                    graph.get_average_distance_source_destinations(possibility,
                                                                   remaining_players_positions)
                aggressive_positions_map[possibility] = average_distance
