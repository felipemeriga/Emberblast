...
class BotDecisioning(IBotDecisioning):
    ...
    def find_foes_within_damage_range(self) -> Optional[IPlayer]:
        ...
        if self.current_bot.job.attack_type == "ranged":
            attack_range_possibilities.extend(
                self.game.game_map.graph.get_available_nodes_in_range(self.current_bot.position, attack_range)
            )
        skill_range = self.get_skills_average_range()
        skill_range_possibilities = self.game.game_map.graph.get_available_nodes_in_range(
            self.current_bot.position, skill_range
        )

        for foe in self.prioritized_foes:
            if (
                foe.position in attack_range_possibilities
                and foe.is_hidden() is False
                or foe.position in skill_range_possibilities
                and foe.is_hidden() is False
            ):
                possible_foe = foe
                break
        ...
        if found_items is not None:
            for item in found_items:
                ...
                self.communicator.informer.render(
                    ItemFoundEvent(
                        player_name=self.current_bot.name, found=True, item_tier=item.tier, item_name=item.name
                    )
                )
        else:
            self.communicator.informer.render(ItemFoundEvent(player_name=self.current_bot.name, found=False))
        return
    ...
    def find_foes(self, movement_possibilities: List[str]) -> Optional[IPlayer]:
        ...
        attack_range_possibilities = self.game.game_map.graph.get_available_nodes_in_range(
            self.current_bot.position, attack_range
        )
        skill_range = self.get_skills_average_range()
        skill_range_possibilities = self.game.game_map.graph.get_available_nodes_in_range(
            self.current_bot.position, skill_range
        ...
        for foe in self.prioritized_foes:
            if (
                foe.position in movement_possibilities
                or foe.position in attack_range_possibilities
                or foe.position in skill_range_possibilities
            ):
                possible_foe = foe
                break
    ...
    def sort_foes_by_priority(self) -> None:
        ...
        for player in remaining_players:
            distance = self.game.game_map.graph.get_shortest_distance_between_positions(
                self.current_bot.position, player.position
            )
    ...
    def prepare_execute_skill(self, skill: ISkill) -> None:
        ...
        if skill.kind == "recover":
            ...
            prefix = "recover"
        if skill.area > 0:
            foes = self.get_affected_players_area_skill(self.possible_foe, self.prioritized_foes, skill.area)
            if len(foes) > 0:
                self.communicator.informer.render(
                    AreaDamageEvent(skill_name=skill.name, skill_kind=skill.kind, affected_players=foes)
                )
        ...
        self.communicator.informer.render(
            DiceRollEvent(
                player_name=self.current_bot.name,
                result=dice_result,
                kind=prefix,
    ...
    def decide_best_defensive_action(self) -> None:
        ...
        if isinstance(best_option, IHealingItem):
            ...
            self.communicator.informer.render(
                UseItemEvent(
                    player_name=self.current_bot.name, item_name=best_option.name, target_name=self.current_bot.name
                )
            )
    ...
    def attack(self) -> None:
        ...
        if (
            self.current_bot.job.attack_type == "melee" and self.possible_foe.position == self.current_bot.position
        ) or (
            self.current_bot.job.attack_type == "ranged" and self.possible_foe.position in attack_range_possibilities
        ):
        ...
        for skill in self.current_bot.skills:
            if (
                skill.cost < self.current_bot.mana
                and skill.kind == "inflict"
                and self.game.game_map.graph.is_target_in_range(
                    self.current_bot.position, skill.ranged, self.possible_foe.position
                )
            ):
                attack_possibilities_dict[skill] = (
                    self.current_bot.get_attribute_real_value(skill.base_attribute) + skill.base
                )
        ...
        if best_attack == "attack":
            ...
            if damage > 0:
                ...
                self.communicator.informer.render(
                    DamageEvent(
                        attacker_name=self.current_bot.name,
                        target_name=self.possible_foe.name,
                        damage=damage,
                        target_alive=self.possible_foe.is_alive(),
                ...
                self.communicator.informer.render(XPEarnedEvent(player_name=self.current_bot.name, xp=experience))
                ...
                self.communicator.informer.render(
                    MissedAttackEvent(attacker_name=self.current_bot.name, target_name=self.possible_foe.name)
                )
            return
    ...
    def move(self) -> None:
        ...
        if self.current_play_style == IPlayingMode.DEFENSIVE:
            safer_positions_map = self.game.game_map.graph.get_average_distances_sources_destinations_map(
                possibilities, remaining_players_positions
            )
        ...
        if self.current_play_style == IPlayingMode.AGGRESSIVE:
            ...
            if possible_foe is None:
                aggressive_positions_map = self.game.game_map.graph.get_average_distances_sources_destinations_map(
                    possibilities, remaining_players_positions
                )
                sorted_aggressive_positions_tuple = sorted(
                    aggressive_positions_map.items(), key=lambda x: x[1], reverse=False
                )
                ...
                if self.current_bot.job.attack_type == "melee":
                    ...
                    aggressive_possibilities = self.game.game_map.graph.get_available_nodes_in_range(
                        possible_foe.position, attack_range
                    )
    ...
    def select_best_equipment(
        self, equipped: IEquipmentItem, new_equipment: IEquipmentItem
    ) -> Optional[IEquipmentItem]:
        best_choice = {equipped: 0, new_equipment: 0}
    ...
    def equip_item(self) -> None:
        ...
        for equipment in equipments:
            ...
            if self.current_bot.equipment.__getattribute__(equipment.category) is None:
                ...
                best_equip = self.select_best_equipment(
                    self.current_bot.equipment.__getattribute__(equipment.category), equipment
                )
    ...
    def select_playing_mode(self) -> None:
        ...
        if self.current_bot.life > low_life_level:
            ...
            self.current_bot.life < low_life_level
            and len(remaining_players) == 1
            and self.probability_of_damage(remaining_players[0])
        ):
            self.current_play_style = IPlayingMode.AGGRESSIVE
        else:
            self.current_play_style = IPlayingMode.DEFENSIVE
    ...
    async def decide(self, player: IPlayer) -> None:
        ...
        system_prompt = build_personality_prompt(player.name, player.job.get_name(), player.race.get_name())
        ...
        if narration:
            self.communicator.informer.render(NarrationEvent(player_name=player.name, text=narration))
    ...
    def _execute_llm_move(self, player: IPlayer, parsed: Dict) -> bool:
        ...
        if not destination:
            return False
        walkable = self.game.game_map.graph.get_available_nodes_in_range(player.position, player.move_speed)
    ...
    def _execute_llm_item(self, player: IPlayer, parsed: Dict) -> bool:
        ...
        for item in player.bag.get_usable_items():
            if item.name.lower() == item_name.lower():
                ...
                self.communicator.informer.render(
                    UseItemEvent(player_name=player.name, item_name=item.name, target_name=player.name)
                )
                return True
        return False
